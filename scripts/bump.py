...
cargo_paths = ["parser", "python_ext", "toktrie", "toktrie_hf_tokenizers", "toktrie_tiktoken", "toktrie_hf_downloader"]
version_pattern = r'\nversion\s*=\s*"(\d+\.\d+\.\d+)([^"]*)"'
...
def check_in_and_tag(version):
    subprocess.run(
        ["git", "add", pyproject_path] + [p + "/Cargo.toml" for p in cargo_paths] + ["Cargo.lock"] + auto_commit,
        check=True,
    )
...
def generate_changelog(version: str) -> str:
    try:
        result = subprocess.run(
            ["auto-changelog", "--stdout", "--commit-limit", "false", "--unreleased-only"],
...
def main():
    ...
    new_version = input(f"Enter new version (default: {suggested_version}): ") or suggested_version
