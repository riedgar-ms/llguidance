...
#include <algorithm>
#include <utility>
#include "llguidance/llguidance.hpp"
#include "cbison.hpp"

int main() {
    // initialize tokenizer and factory
    llguidance::LLTokenizer t("byte");
    intptr_t f_addr = t.copy_as_cbison_factory();
    cbison::Factory f(f_addr);
    ...
    std::vector<uint32_t> row2(mask.begin() + 2*words, mask.begin() + 3*words);
    assert(row0 == mask2);
    assert(row2 == mask2);
